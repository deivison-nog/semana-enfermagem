...
- Checkout transparente Pix do Mercado Pago para inscrição de **R$ 10,00** (pagamento único);
...
3. O sistema cria o pagamento Pix via API de Payments do Mercado Pago usando `MERCADOPAGO_ACCESS_TOKEN`.
4. A inscrição é salva no banco SQL com `external_reference` e `payment_id`.
5. O usuário é redirecionado para `payment_pix.php`, onde vê QR Code e código copia-e-cola sem sair do site.
6. O sistema envia `notification_url` para o Mercado Pago (por padrão `APP_URL/payment_webhook.php`).
7. No webhook e na verificação periódica, o sistema atualiza status, `payment_id` e `payment_date` na tabela `registrations`.
...
- `process_registration.php`: validação + criação do Pix + gravação da inscrição
- `payment_pix.php`: tela interna de pagamento com QR Code Pix e polling de status
